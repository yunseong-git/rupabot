...
export class PostsService {
    ...
    async findAllPosts(query: PostQueryDto): Promise<PostDocument[]> {
        ...
        const sortOption: Record<string, SortOrder> =
            sort === 'like'
                ? { likeCount: -1 }
                : { createdAt: -1 };
        const posts = this.findManyPosts(type, limit, skip, sortOption)
        return posts;
    }
    ...
    async deletePost(postId: string, userId: string): Promise<PostDocument> {
        const isAuthor = await this.isAuthor(userId, postId);
        if (!isAuthor) throw new UnauthorizedException('삭제 권한이 없습니다.');

        const deleted = await this.postModel.findByIdAndUpdate(postId, { isDeleted: true });
    ...
    */
    ...
    async findLikedPostsIds(userId: string): Promise<Object> {
        const likedPostIds = await this.postLikeModel
            .find({ userId })
            .select('postId')
            .lean(); // ← lean()을 써주면 plain JS 객체로 빠름
    ...
    private async findManyPosts(condition: any, limit: number, skip: number, options?: FindOptions,): Promise<PostDocument[]> {
        const query = this.postModel
            .find(condition)
            .select('title viewCount likeCount createdAt tag')
