...
export class PostCommandService {
  ...
  /**게시물 작성*/
  async createPost(dto: CreatePostDto, userId: string): Promise<string> {
  ...
  async selfDeletePost(postId: string, deletedAt: Date, deletedBy: string): Promise<boolean> {
    const updated = await this.postModel.updateOne(
      { _id: postId },
      { $set: { isDeleted: true, likecount: 0, deletedAt: deletedAt, deletedBy: deletedBy } },
    );
    if (updated.matchedCount === 0) throw new NotFoundException('해당 게시물 없음');
    if (updated.modifiedCount === 0) throw new BadRequestException('변경 된 내용 없음');
