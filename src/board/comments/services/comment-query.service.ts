...
export class CommentQueryService {
  ...
  async findAllCommentsByPost(query: PostQueryDto.WithComments): Promise<(PlainComment & { userTag: UserTag })[]> {
  ...
  async findParentsCommentsByPost(query: PostQueryDto.WithComments): Promise<(PlainComment & { userTag: UserTag })[]> {
    const { postId, sort, limit = 10, skip = 0 } = query;
  ...
  /**부모 댓글의 답글들 반환*/
  async findReplyCommentsByParent(query: CommentQueryDto.Reply): Promise<(PlainComment & { userTag: UserTag })[]> {
  ...
  private async findManyComments(filter: any, option: QueryOptions, selection: any): Promise<(PlainComment & { userTag: UserTag })[]> {
  ...
  /**해당 유저가 작성한 모든 댓글 확인*/
  async findMyComments(query: CommentQueryDto.My, userId: string): Promise<PlainComment[]> {
  ...
  async isAuthor(userId: string, commentId: string): Promise<boolean> {
    ...
    if (!comment.authorId.equals(userId)) { throw new ForbiddenException('작성자만 접근할 수 있습니다.'); }
    else return true;
  }
  ...
  async isParents(id: string): Promise<boolean> {
    ...
    if (!comment) { throw new NotFoundException('존재하지 않거나 삭제된 댓글입니다.'); }
    if (comment.pId) { throw new BadRequestException('잘못된 요청입니다.'); }
